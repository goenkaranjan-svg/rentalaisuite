    ...
    <div className="space-y-8 animate-in">
      ...
      <Card className="border-slate-200 shadow-sm overflow-hidden">
        <div className="space-y-3 p-4 md:hidden">
          {leases?.map((lease) => (
            <div key={lease.id} className="rounded-lg border border-slate-200 p-4 space-y-3">
              <div className="flex items-start justify-between gap-3">
                <div>
                  <p className="font-medium text-slate-900">
                    Tenant {lease.tenantId ? `${lease.tenantId.substring(0, 8)}...` : "N/A"}
                  </p>
                  <p className="text-sm text-slate-500">Unit #{lease.propertyId}</p>
                </div>
              ...
              <div className="text-sm text-slate-600">
                <p>{format(new Date(lease.startDate), "MMM yyyy")} - {format(new Date(lease.endDate), "MMM yyyy")}</p>
                <p className="font-mono font-medium text-slate-900 mt-1">${Number(lease.rentAmount).toLocaleString()}</p>
              </div>
          ...
          {leases?.length === 0 && <p className="text-center text-slate-500 py-8">No leases found.</p>}
        </div>
      ...
      <Card className="border-slate-200 shadow-sm overflow-hidden">
        ...
        <div className="space-y-3 p-4 md:hidden">
          {(pipeline ?? []).slice(0, 12).map((item) => (
            <div key={item.leaseId} className="rounded-lg border border-slate-200 p-4 space-y-2">
              <div className="flex items-start justify-between gap-3">
                <p className="font-medium text-slate-900">#{item.leaseId} · Property {item.propertyId}</p>
                <Badge
                  variant="outline"
                  className={
              ...
              <p className="text-sm text-slate-600">Days to end: {item.daysUntilEnd}</p>
              <p className="text-sm text-slate-600">{item.nextAction}</p>
            </div>
          ))}
          {(pipeline ?? []).length === 0 && <p className="text-center text-slate-500 py-8">No renewal pipeline items available.</p>}
        </div>
        <div className="hidden md:block">
          <Table>
            <TableHeader>
